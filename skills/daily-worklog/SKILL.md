...
Then one question, naming the count: post these N worklogs? Nothing is sent until the user says yes. One yes covers the whole day. Any change: present the whole table again and ask again.
...
| "The 6/2 ticket split is inferred from the daily note's timestamps, not exact — flagged as a soft spot with the fallback (fold all 8 h into [one ticket])" | Same shortcut with a fallback attached. The fallback is the gap question, not a fold |
...
| "the … split (1.25h/7.00h) is an estimate from the daily note's timeline, not a timestamped calculation" | An estimate presented as a row is a guess the invoice has to defend. Evidence sizes the row; anything left over is `unattributed` |
...
| "This workflow drafts only … so a billing record never goes to Jira without your review" | Step 5 is that review: the whole table, then one question naming the count |
