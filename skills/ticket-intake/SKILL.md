...
2. **Vault, frontmatter first.** Notes with the key in `jira`: handoffs (read Gated), open questions, open conflicts, decisions (accepted is a constraint, proposed is unsettled), research notes. Then the meetings, by script: `bash scripts/ticket-intake-meetings.sh <vault> <noun>...` from the plugin root (two directories above this skill's folder), with three or more nouns from the ticket's summary and scope table — field and feature names as singular stems, such as `dispatch pallet "gross weight"`. It prints one line per matching sentence, or one no-match line. That output is the `Meetings checked` block of the note (section 1) and the reading list: open every note it names. A sentence that sizes, scopes, or defers the ticket differently from its description is a contradiction (What else gets created), including when the description was rewritten after that meeting: the people in the room have not heard the new scope until someone raises it. Repo dossiers matching the ticket's names or paths: clone path, build notes, owners. Glossary misses. `Meta/Intake Checks.md` if it exists: run each line as written.
...
Look first for an existing note with `type: question` or `type: conflict` whose `jira` carries the key and that covers the point; link it instead of creating another. A review, research, or handoff note that mentions the same contradiction is a source the conflict note cites, not a replacement for it.
...
| Opening only the listed notes whose titles sound related | The list is the reading list; the sentence that resizes the ticket sits in a note about docks and doors |
| "Dated before the re-scope, superseded by the build — no action, noted for the record" | Superseded is a status the conflict note carries (`obsolete`, one line why), not a reason to skip it; until it exists nobody raises the change with the people who sized the ticket |
| "Already flagged as wrong in the review note — no new note needed" | Only a `type: conflict` or `type: question` note counts as existing; a review is a source the conflict note cites |
