...
4. The first page's host must match a pattern in config `test_hosts`. No match: run unrecorded; the note names the host and says why. Key missing: no host is allowed, so the run is unrecorded too — say so and tell the user to add `test_hosts` to the config note. Write nothing under `<folders.attachments>/<KEY>/` for that run — no screenshot, no video, no partial file of any kind. The note's own words are the only evidence an unlisted host gets.
