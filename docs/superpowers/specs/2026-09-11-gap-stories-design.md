...
> **Amended 2026-09-15.** PFD-66644 "View an outbound appointment with its linked Orders (order catalog projection + seeding)" was created on 2026-09-15 and now covers the seeding gap. Fixture B no longer yields a creatable `split PFD-66407` story; the duplicate check should find PFD-66644 instead. The re-baselined expected results live in `docs/superpowers/baselines/gap-stories.md` (commit 2f78cd6), which is the source of truth for every check. The check tables below are superseded where the two disagree. **Also amended:** the skill does not re-run ticket-intake when the Jira ticket's `updated` date is later than the note's `Checked` date. "1. Read the intake note" above lists that as one of three cases for a re-run; `skills/gap-stories/SKILL.md` instead says so in chat and in the Summary, works from the note as it stands, and re-runs only if the user asks. The skill's version is the one that holds, and that bullet is superseded. An automatic re-run rewrites the intake note on every run, and checks A17, A18 and B8 read that note, so all three fail. Do not change the skill back.
