...
**Settled 2026-09-15:** `20` `playwright-cli` bash calls in one full two-criterion walkthrough on the dense fixture, against a floor of 15 (16 with the overlay) for the sparse one; a real client page will be higher. Each verb costs about 5s of round-trip latency (Task 1: 11 verbs produced a 49s recording), so a walkthrough this size costs both a permission-prompt count and roughly a minute and a half of wall clock before any judgment even starts. Invocation form: `env C form`. Prompt behaviour not measured interactively; the count alone carries the instruction. Claude Code's documented Bash-matching rules split `cd "$REC" && playwright-cli …` into two subcommands at `&&`, and a rule must match each independently: `Bash(playwright-cli:*)` covers the `playwright-cli` half, but the `cd` half is auto-approved as a built-in read-only command only when `$REC` sits inside the session's working directory or an already-added directory — outside that, it still needs its own approval, so "one entry covers it" for the compound form is conditional, not guaranteed. `env -C "$REC" playwright-cli …` is a single, unsplit command whose first token is `env`, which is not in Claude Code's built-in stripped-wrapper list (`timeout`, `time`, `nice`, `nohup`, `stdbuf`, `command`, `builtin`, `noglob`), so `Bash(playwright-cli:*)` does not match it at all — the entry that actually silences it is `Bash(env -C * playwright-cli *)`. So `SKILL.md`'s Before-recording section tells the user to add `Bash(env -C * playwright-cli *)` — not `Bash(playwright-cli:*)` — the way it already tells them to install ffmpeg. This `env C form` verdict supersedes the literal `cd` mechanism that Decision 5 (`### 5. Run from the recording directory, one named session per key`) and the SKILL.md-changes table's added-section bullet ("the working directory and the `cd` before every call") both still name; the binding requirement in both places is that the CLI runs with the recording directory as its working directory, not that the word `cd` appears.
